...
# Funció per convertir DMS a decimal
def dms_to_decimal_simple(dms_str):
    ...
    try:
        ...
        segundos_str = minutos_part[1].replace('"', '').strip()
...
df = pd.read_csv('Mar12.csv', header=None)

# Obtenim les columnes d'azimut i altitud 
az_stell = df[2].tolist()[1:] 
alt_stell = df[3].tolist()[1:]

# Convertim totes les dades a decimals
az_stell_dec = [dms_to_decimal_simple(x) for x in az_stell]
alt_stell_dec = [dms_to_decimal_simple(x) for x in alt_stell]

# Filtrar valors invalids (nan)
az_stell_dec = np.array(az_stell_dec)
alt_stell_dec = np.array(alt_stell_dec)

valid_indices = ~np.isnan(az_stell_dec) & ~np.isnan(alt_stell_dec)
az_stell_dec = az_stell_dec[valid_indices]
alt_stell_dec = alt_stell_dec[valid_indices]

# Obliqüitat de l'eclíptica
ε = np.radians(23.44)  

# Funció per calcular l'anomalia mitjana (D) 
def calculate_D(y, d):
...
def solar_declination(day):
    return 23.44 * np.sin(2 * np.pi * (day - 81) / 365)
...
def analemma(lat, lon, hour, year=2025):
    ...
    plt.plot(azimuts, altures, color='r', label='Analema del codi')
    plt.plot(az_stell_dec, alt_stell_dec, marker='o', linestyle='-', markersize=1, label='Dades Stellarium')
...
#Analema des de Bellaterra (41.5°N, 2.107°E), hora local 12:00h
